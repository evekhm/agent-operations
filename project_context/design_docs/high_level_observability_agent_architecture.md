...
The **Observability Agent** is an autonomous, AI-powered Site Reliability Engineer (SRE) designed specifically to analyze data generated by the [**BigQuery Agent Analytics plugin for ADK**](https://google.github.io/adk-docs/integrations/bigquery-agent-analytics/).
...
This agent moves beyond static dashboards by deeply integrating with the comprehensive event data captured by the BigQuery plugin. This data is not just basic telemetry; it is a rich dataset extended with **ADK events and execution contexts**, tracking entire LLM reasoning traces, tool executions, state deltas, and multi-agent orchestration overhead. The Observability Agent uses this granular, agent-specific data to dynamically establish performance baselines, construct mathematical hypotheses about system bottlenecks, and produce actionable, data-backed insights.

## 1. Core Purpose and Persona
The Observability Analyst Agent acts as an autonomous Senior Site Reliability Engineer (SRE) and Lead Data Scientist for the AI agent ecosystem. Its primary goal is to proactively monitor, investigate, and report on the health, stability, and performance of the system. Instead of interrogating generic telemetry data, it specifically leverages the BigQuery Agent Analytics plugin's ADK event data to hypothesize potential issues (e.g., KV cache fragmentation, reasoning bottlenecks, sequential tool execution faults), test these hypotheses rigorously using data, and synthesize its findings into a highly structured "Gold Standard" report.
