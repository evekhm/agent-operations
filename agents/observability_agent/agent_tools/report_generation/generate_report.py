...
class ReportGenerator:
    def __init__(self, data: Dict[str, Any], config: Dict[str, Any], base_dir: str = None):
        ...
        self.quality_eval_results = data.get('quality_eval_results', {})
        self.quality_ai_summary = data.get('quality_ai_summary', '')
        self.execution_trees = data.get('execution_trees', {})
...
- **[Response Quality Analysis](#response-quality-analysis):** AI-powered evaluation of whether agent responses genuinely answer user questions, with a quality scorecard, breakdown by category, and example sessions showing actual user queries and agent replies.
...
- **Granular Breakdowns:** Browse the [Agent Details](#agent-details), [Tool Details](#tool-details), and [Model Details](#model-details) for raw volume, traffic distribution, token breakdowns, and sequential latency charts over time."""
    ...
    def _render_executive_summary(self):
        ...
        # Inject quality data so the AI agent can include it in the summary
        self._render_quality_overview()

        self.report_content.append("\n---\n")

    def _render_quality_overview(self):
        """Render a quality overview card for the executive summary."""
        quality_data = self.quality_eval_results
        if not quality_data or not isinstance(quality_data, dict) or quality_data.get("total_sessions", 0) == 0:
            return
        ...
        usefulness_dist = distributions.get("response_usefulness", {})
        grounding_dist = distributions.get("task_grounding", {})

        meaningful = usefulness_dist.get("meaningful", 0)
        unhelpful = usefulness_dist.get("false_positive", 0)
        partial = usefulness_dist.get("partial", 0)
        evaluated = meaningful + unhelpful + partial

        grounded = grounding_dist.get("grounded", 0)
        ungrounded = grounding_dist.get("ungrounded", 0)
        grounding_evaluated = grounded + ungrounded + grounding_dist.get("no_tool_needed", 0)

        quality_rate = (meaningful / evaluated * 100) if evaluated > 0 else 0.0
        unhelpful_rate = (unhelpful / evaluated * 100) if evaluated > 0 else 0.0
        grounding_rate = (grounded / grounding_evaluated * 100) if grounding_evaluated > 0 else 0.0

        quality_emoji = "🟢" if quality_rate >= 80 else ("🟡" if quality_rate >= 60 else "🔴")
        unhelpful_emoji = "🟢" if unhelpful_rate < 10 else ("🟡" if unhelpful_rate < 25 else "🔴")
        grounding_emoji = "🟢" if grounding_rate >= 80 else ("🟡" if grounding_rate >= 60 else "🔴")

        self.report_content.append("\n### Response Quality Overview\n\n")
        self.report_content.append(
            f"Based on **{evaluated}** of **{total}** sessions evaluated "
            f"by AI quality analysis:\n\n"
        )

        self.report_content.append(
            f"| Metric | Status | Value |\n"
            f"|--------|--------|-------|\n"
            f"| Response Quality | {quality_emoji} | **{quality_rate:.1f}%** meaningful ({meaningful} of {evaluated}) |\n"
            f"| Unhelpful Rate | {unhelpful_emoji} | **{unhelpful_rate:.1f}%** ({unhelpful} sessions) |\n"
            f"| Grounding Rate | {grounding_emoji} | **{grounding_rate:.1f}%** grounded ({grounded} of {grounding_evaluated}) |\n"
        )

        if a2a_count:
            self.report_content.append(
                f"\n*{a2a_count} A2A sessions detected (remote agent responses resolved).*\n"
            )

        if unhelpful_rate >= 25:
            self.report_content.append(
                f"\n⚠️ **High unhelpful rate** — {unhelpful} sessions where the agent failed to "
                f"substantively answer the user's question. See [Response Quality Analysis]"
                f"(#response-quality-analysis) for details and example sessions.\n"
            )
        elif unhelpful_rate >= 10:
            self.report_content.append(
                f"\n⚠️ Unhelpful rate above 10% threshold. See [Response Quality Analysis]"
                f"(#response-quality-analysis) for details.\n"
            )
    ...
    def _render_agent_details(self):
        ...
        if isinstance(self.df_agent_models, pd.DataFrame) and not self.df_agent_models.empty:
             ...
             try:
                 pivot_df = self.df_agent_models.pivot(index='agent_name', columns='model_name', values='total_count').fillna(0)
    ...
    def _render_system_bottlenecks(self):
        ...
        if isinstance(self.root_bottlenecks, pd.DataFrame) and not self.root_bottlenecks.empty:
            ...
            # Render execution path trees for slow invocations
            if self.execution_trees:
                self.report_content.append("\n#### Execution Path Breakdown\n\n")
                self.report_content.append(
                    "The trees below show which sub-agents and tools were invoked for each slow trace, "
                    "how long each step took, and whether tools were retried.\n\n"
                )
                for rank_idx, (_, row) in enumerate(df_root.iterrows()):
                    trace_id = row.get('trace_id', '')
                    if trace_id not in self.execution_trees:
                        continue
                    root_name = row.get('root_agent_name', 'root')
                    root_dur = row.get('duration_ms', 0)
                    root_dur_display = f"{root_dur:.1f}s" if isinstance(root_dur, (int, float)) else str(root_dur)
                    self.report_content.append(
                        f"**Rank {rank_idx + 1}** — Trace `{trace_id}`\n\n"
                    )
                    tree_text = self._build_execution_tree(
                        self.execution_trees[trace_id], root_name, root_dur
                    )
                    self.report_content.append(f"```\n{tree_text}```\n\n")
        else:
        ...
        if isinstance(self.llm_bottlenecks, pd.DataFrame) and not self.llm_bottlenecks.empty:
             ...
             if 'agent_duration_ms' in df_llm.columns:
                 df_llm['Agent (s)'] = (pd.to_numeric(df_llm['agent_duration_ms'], errors='coerce').fillna(0) / 1000).round(3)
             if 'root_duration_ms' in df_llm.columns:
                 df_llm['E2E (s)'] = (pd.to_numeric(df_llm['root_duration_ms'], errors='coerce').fillna(0) / 1000).round(3)
        ...
        if isinstance(self.tool_bottlenecks, pd.DataFrame) and not self.tool_bottlenecks.empty:
             ...
             if 'agent_duration_ms' in df_tool.columns:
                 df_tool['agent_duration_s'] = (pd.to_numeric(df_tool['agent_duration_ms'], errors='coerce').fillna(0) / 1000).round(3)
             else:
                 df_tool['agent_duration_s'] = 0.0
    ...
    def _render_per_agent_quality(self, session_results):
        """Render a per-agent quality breakdown table."""
        ...
        for sr in session_results:
            ...
            if agent not in agent_stats:
                agent_stats[agent] = {
                    "total": 0, "meaningful": 0, "unhelpful": 0, "partial": 0,
                    "grounded": 0, "ungrounded": 0, "is_a2a": sr.get("is_a2a", False),
                }
    ...
    def _render_quality_analysis(self):
        ...
        self.report_content.append(
            "This section evaluates the **semantic quality** of agent responses — not just whether "
            "they completed without errors, but whether they actually answered the user's question. "
            "An agent can return HTTP 200 with normal latency yet produce a completely unhelpful "
            "response (e.g., *\"Sorry, I cannot help with that\"* or empty search results dressed "
            "up as an answer). This section uses AI-powered evaluation to detect these cases.\n\n"
        )
        self.report_content.append(
            "**How to read this section:**\n\n"
            "Each session is evaluated on two dimensions:\n\n"
            "1. **Response Usefulness** — Did the agent substantively answer the user's question?\n"
            "   - **Meaningful**: The response directly addresses the question with specific, actionable information.\n"
            "   - **Unhelpful**: The agent technically succeeded but did NOT answer the question "
            "(apologies, \"I don't have that information\", empty results, generic filler).\n"
            "   - **Partial**: The response touches on the topic but is incomplete or only tangentially relevant.\n\n"
            "2. **Task Grounding** — Is the response backed by actual tool/data results?\n"
            "   - **Grounded**: Response is based on data retrieved from the agent's tools.\n"
            "   - **Ungrounded**: Response appears fabricated or based on general knowledge, not tool results.\n"
            "   - **No tool needed**: The question didn't require tool usage.\n\n"
        )

        quality_data = self.quality_eval_results
        if not quality_data or not isinstance(quality_data, dict) or quality_data.get("total_sessions", 0) == 0:
            self.report_content.append(
                "*No response quality evaluation data is available for this time period. "
                "Ensure the `bigquery_agent_analytics` SDK is installed and sessions exist "
        ...
        total = quality_data["total_sessions"]
        distributions = quality_data.get("category_distributions", {})
        session_results = quality_data.get("session_results", [])
        details = quality_data.get("details", {})
        a2a_count = quality_data.get("a2a_sessions", 0)
        # --- Sessions analyzed info ---
        a2a_info = ""
        if a2a_count:
            a2a_info = f" ({a2a_count} A2A sessions detected)"
        self.report_content.append(
            f"**{total} sessions analyzed**{a2a_info}\n\n"
        )

        # --- Quality Scorecard ---
        self.report_content.append("### Quality Scorecard\n\n")

        usefulness_dist = distributions.get("response_usefulness", {})
        grounding_dist = distributions.get("task_grounding", {})

        meaningful_count = usefulness_dist.get("meaningful", 0)
        unhelpful_count = usefulness_dist.get("false_positive", 0)
        partial_count = usefulness_dist.get("partial", 0)
        evaluated = meaningful_count + unhelpful_count + partial_count
        quality_rate = (meaningful_count / evaluated * 100) if evaluated > 0 else 0.0
        unhelpful_rate = (unhelpful_count / evaluated * 100) if evaluated > 0 else 0.0

        grounded_count = grounding_dist.get("grounded", 0)
        ungrounded_count = grounding_dist.get("ungrounded", 0)
        grounding_evaluated = grounded_count + ungrounded_count + grounding_dist.get("no_tool_needed", 0)
        grounding_rate = (grounded_count / grounding_evaluated * 100) if grounding_evaluated > 0 else 0.0

        # Status emoji based on quality
        quality_status = "🟢" if quality_rate >= 80 else ("🟡" if quality_rate >= 60 else "🔴")
        grounding_status = "🟢" if grounding_rate >= 80 else ("🟡" if grounding_rate >= 60 else "🔴")

        scorecard_df = pd.DataFrame([
            {
                "Metric": "Response Quality Rate",
                "Status": quality_status,
                "Value": f"{quality_rate:.1f}%",
                "Detail": f"{meaningful_count} meaningful out of {evaluated} evaluated sessions",
            },
            {
                "Metric": "Unhelpful Response Rate",
                "Status": "🟢" if unhelpful_rate < 10 else ("🟡" if unhelpful_rate < 25 else "🔴"),
                "Value": f"{unhelpful_rate:.1f}%",
                "Detail": f"{unhelpful_count} sessions where agent failed to answer the question",
            },
            {
                "Metric": "Grounding Rate",
                "Status": grounding_status,
                "Value": f"{grounding_rate:.1f}%",
                "Detail": f"{grounded_count} responses backed by actual tool data",
            },
        ])
        scorecard_df = self.formatter.standardize_table_formatting(scorecard_df)
        self.report_content.append(scorecard_df.to_markdown(index=False))
        self.report_content.append("\n\n")

        self.report_content.append(f"*{total} total sessions in dataset, {evaluated} successfully classified.*\n\n")

        if unhelpful_rate > 25:
            self.report_content.append(
                f"**CRITICAL:** {unhelpful_rate:.0f}% of agent responses are unhelpful. "
                "Users are asking questions the agent cannot answer — either due to missing "
                "data sources, incorrect routing, or tools that silently return empty results.\n\n"
            )
        elif unhelpful_rate > 10:
            self.report_content.append(
                f"**WARNING:** {unhelpful_rate:.0f}% unhelpful response rate exceeds the 10% threshold.\n\n"
            )
        ...
        if a2a_count:
            self.report_content.append(
                f"*{a2a_count} sessions involved remote A2A agents.*\n\n"
            )

        # --- AI Summary ---
        if self.quality_ai_summary:
            self.report_content.append("### AI Analysis Summary\n\n")
            self.report_content.append(self.quality_ai_summary + "\n\n")

        # --- Per-Agent Quality Breakdown ---
        self._render_per_agent_quality(session_results)

        # --- Breakdown by Category ---
        self.report_content.append("### Quality Breakdown\n\n")
        ...
        category_colors = {
            ...
            "Grounded": "#2196F3",
            "Ungrounded": "#F44336",
            "No Tool Needed": "#9E9E9E",
        }

        for metric_name, dist in distributions.items():
            display_name = "Response Usefulness" if metric_name == "response_usefulness" else "Task Grounding"
            self.report_content.append(f"#### {display_name}\n\n")
            ...
            for category, count in sorted(dist.items(), key=lambda x: -x[1]):
                ...
                display_cat = "Unhelpful" if category == "false_positive" else category.replace("_", " ").title()
                rows.append({
                    "Category": display_cat,
                    "Sessions": count,
                    "Rate": f"{pct:.1f}%",
                })
                chart_data[display_cat] = count
            df_dist = pd.DataFrame(rows)
            df_dist = self.formatter.standardize_table_formatting(df_dist)
            self.report_content.append(df_dist.to_markdown(index=False))
            self.report_content.append("\n\n")

            # Generate pie chart
            chart_series = pd.Series(chart_data)
            chart_path = self.chart_gen.generate_pie_chart(
                data=chart_series,
                title=display_name,
                filename=f"quality_{metric_name}.png",
                colors=category_colors,
            )
            if chart_path:
                rel_path = os.path.relpath(chart_path, self.report_dir)
                self.report_content.append(f"![{display_name}]({rel_path})\n\n")

        # --- Example Sessions ---
        # Collect sessions by category
        unhelpful_sessions = []
        meaningful_sessions = []
        partial_sessions = []
        for sr in session_results:
            for mr in sr.get("metrics", []):
                if mr.get("metric_name") == "response_usefulness":
                    if mr.get("category") == "false_positive":
                        unhelpful_sessions.append(sr)
                    elif mr.get("category") == "meaningful":
                        meaningful_sessions.append(sr)
                    elif mr.get("category") == "partial":
                        partial_sessions.append(sr)
                    break

        self.report_content.append("### Example Sessions\n\n")
        self.report_content.append(
            "Below are representative examples from each quality category, showing the actual "
            "user question, the agent's response, and the AI evaluator's assessment.\n\n"
        )

        def _render_example_sessions(sessions, category_label, max_examples=5):
            if not sessions:
                return
            self.report_content.append(f"#### {category_label} Responses\n\n")
            shown = 0
            for sr in sessions:
                ...
                answered_by = sr.get("answered_by", "")
                agent_tag = f" → **{answered_by}**" if answered_by else ""
                self.report_content.append(f"**Session** `{sr['session_id']}`{a2a_tag}{agent_tag}\n\n")

                if user_msg:
                    self.report_content.append(f"**User asked:** *{user_msg}*\n\n")
                if agent_resp:
                    self.report_content.append(f"**Agent replied:** {agent_resp}\n\n")

                # Show usefulness and grounding verdicts with emoji
                usefulness_emoji = {"meaningful": "✅", "false_positive": "❌", "partial": "⚠️"}
                grounding_emoji = {"grounded": "✅", "ungrounded": "❌", "no_tool_needed": "➖"}
                for mr in sr.get("metrics", []):
                    if mr.get("metric_name") == "response_usefulness":
                        cat = mr.get("category", "")
                        emoji = usefulness_emoji.get(cat, "❓")
                        label = {"meaningful": "Helpful", "false_positive": "Not Helpful", "partial": "Partial"}.get(cat, cat)
                        self.report_content.append(f"{emoji} **Usefulness:** {label}\n\n")
                        if mr.get("justification"):
                            self.report_content.append(f"*{mr['justification'][:400]}*\n\n")
                    elif mr.get("metric_name") == "task_grounding":
                        cat = mr.get("category", "")
                        emoji = grounding_emoji.get(cat, "❓")
                        label = {"grounded": "Grounded", "ungrounded": "Not Grounded", "no_tool_needed": "No Tool Needed"}.get(cat, cat)
                        self.report_content.append(f"{emoji} **Grounding:** {label}\n\n")

                self.report_content.append("---\n\n")
                shown += 1

        num_examples = self.pres_config.get("num_quality_examples", 10)
        _render_example_sessions(unhelpful_sessions, "Unhelpful", max_examples=num_examples)
        _render_example_sessions(meaningful_sessions, "Meaningful", max_examples=num_examples)
        _render_example_sessions(partial_sessions, "Partial", max_examples=num_examples)
    ...
    @staticmethod
    def _build_execution_tree(spans: list, root_name: str, root_dur) -> str:
        """Builds a text-based execution tree from spans for a single trace."""
        if not spans:
            return f"{root_name} ({root_dur:.1f}s)\n  (no sub-spans found)\n"
        ...
        # Find root spans (spans whose parent is not in span_map)
        root_spans = []
        for s in spans:
            pid = s.get('parent_span_id', '')
            if pid not in span_map:
                root_spans.append(s['span_id'])

        # Deduplicate while preserving order
        seen = set()
        root_spans_unique = []
        for sid in root_spans:
            if sid not in seen:
                seen.add(sid)
                root_spans_unique.append(sid)
        root_spans = root_spans_unique

        lines = []
        dur_display = f"{root_dur:.1f}s" if isinstance(root_dur, (int, float)) else str(root_dur)
        lines.append(f"{root_name} ({dur_display})")

        def render_span(sid, prefix="", is_last=True):
            s = span_map.get(sid)
            if not s:
                return
            name = s.get('name', '?')
            ...
            type_tag = f"[{span_type}] " if span_type else ""
            result_hint = ""
            if span_type == "tool" and s.get('tool_result'):
                result_text = str(s['tool_result'])[:80]
                result_hint = f' → "{result_text}"'

            lines.append(f"{prefix}{connector}{type_tag}{name} ({dur_s:.1f}s) {status_icon}{result_hint}")

            child_ids = children.get(sid, [])
            child_prefix = prefix + ("    " if is_last else "│   ")
            for i, cid in enumerate(child_ids):
                render_span(cid, child_prefix, i == len(child_ids) - 1)

        for i, sid in enumerate(root_spans):
            render_span(sid, "  ", i == len(root_spans) - 1)

        # Count tool retries
        tool_names = [s.get('name') for s in spans if s.get('span_type') == 'tool']
        from collections import Counter
        tool_counts = Counter(tool_names)
        retried = {name: count for name, count in tool_counts.items() if count > 1}
        if retried:
            lines.append("")
            lines.append("  Retried tools:")
            for name, count in retried.items():
