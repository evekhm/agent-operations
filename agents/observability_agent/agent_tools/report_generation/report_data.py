...
class ReportDataManager:
    ...
    async def fetch_raw_llm_data(self, time_range: str = "24h", limit: int = MAX_RAW_RECORDS_LIMIT):
        """Fetches raw LLM event data using fetch_correlation_data tool."""
        try:
            raw_json = await fetch_correlation_data(time_range=time_range, limit=limit)
    ...
    async def fetch_raw_agent_data(self, time_range: str = "24h", limit: int = MAX_RAW_RECORDS_LIMIT):
    ...
    async def fetch_all_data(self) -> Dict[str, Any]:
        ...
        data = await perform_inline_rca(data, limit=rca_limit)
