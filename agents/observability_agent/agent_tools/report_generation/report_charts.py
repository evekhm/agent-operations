...
class ChartGenerator:
    ...
    def generate_model_latency_category_chart(self, df: pd.DataFrame, x_col: str, hue_col: str, title: str, filename: str, order: list = None, figsize: tuple = None):
        if df.empty: return None
        if figsize is None:
            # Make the chart wider to improve readability of bars
            figsize = self._get_figsize(14, 6)
        plt.figure(figsize=figsize)
        
        try:
            sns.countplot(data=df, x=x_col, hue=hue_col, order=order, palette="muted", edgecolor='white', alpha=0.9)
        except Exception as e:
            logger.warning(f"Failed to generate countplot: {e}")
            return None
